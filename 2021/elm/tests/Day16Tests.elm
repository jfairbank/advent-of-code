module Day16Tests exposing (suite)

import Day16 exposing (puzzle1, puzzle2)
import Expect
import Test exposing (..)


suite : Test
suite =
    describe "Day16"
        [ describe "puzzle1"
            [ test "solves example inputs" <|
                \_ ->
                    let
                        ( inputs, expectedValues ) =
                            [ ( "8A004A801A8002F478", 16 )
                            , ( "620080001611562C8802118E34", 12 )
                            , ( "C0015000016115A2E0802F182340", 23 )
                            , ( "A0016C880162017C3686B18A3D4780", 31 )
                            ]
                                |> List.map (Tuple.mapSecond Just)
                                |> List.unzip
                    in
                    List.map puzzle1 inputs
                        |> Expect.equalLists expectedValues
            , test "solves puzzle input" <|
                \_ ->
                    puzzle1 puzzleInput
                        |> Expect.equal (Just 821)
            ]
        , describe "puzzle2"
            [ test "solves example inputs" <|
                \_ ->
                    let
                        ( inputs, expectedValues ) =
                            [ ( "C200B40A82", 3 )
                            , ( "04005AC33890", 54 )
                            , ( "880086C3E88112", 7 )
                            , ( "CE00C43D881120", 9 )
                            , ( "D8005AC2A8F0", 1 )
                            , ( "F600BC2D8F", 0 )
                            , ( "9C005AC2F8F0", 0 )
                            , ( "9C0141080250320F1802104A08", 1 )
                            ]
                                |> List.map (Tuple.mapSecond Just)
                                |> List.unzip
                    in
                    List.map puzzle2 inputs
                        |> Expect.equalLists expectedValues
            , test "solves puzzle input" <|
                \_ ->
                    puzzle2 puzzleInput
                        |> Expect.equal (Just 2056021084691)
            ]
        ]


puzzleInput : String
puzzleInput =
    ""
